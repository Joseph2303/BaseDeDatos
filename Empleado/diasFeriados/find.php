...
<head>
  ...
  <script>
    $(document).ready(function() {
      $('input[name="idDiasFeriados"]').on('input', function() {
        var parametroBuscar = $(this).val();

        $.ajax({
          type: 'POST',
          url: 'tabla.php',  // Cambia esto al archivo que maneja la búsqueda
          data: { buscar: 'Buscar', idDiasFeriados: parametroBuscar },
...
<body style="background-image: url(img/fondo-degradado-tonos-verdes_23-2148387744.avif); background-repeat:  no-repeat; background-size: cover;">
  <div class="container p-4">
    <div class="row justify-content-center">
      <div class="col-md-8">
        <div class="card">
          <div class="card-header">Buscar Día Feriado</div>
          <div class="card-body">
            <form method="POST" action="">
              <div class="form-group row">
                <label for="idDiasFeriados" class="col-md-4 col-form-label custom-label">Día Feriado</label>
                <div class="col-md-8">
                  <input type="text" name="idDiasFeriados" class="form-control" placeholder="Digite la descripción del Día Feriado" required>
                </div>
              </div>
